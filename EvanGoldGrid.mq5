...
input double    InpMaxDrawdownPercent = 2.0;
...
input bool      InpTrailingStopEnabled      = false;
input double    InpProfitTrailingPercent    = 50.0;

input group "=== Grid Shift ==="
input bool      InpAutoShiftGrid    = false;
input int       InpShiftTriggerBars = 3;
input double    InpBoundaryTriggerPercent = 2.0;
input double    InpShiftGridPercent = 5.0;
...
int OnInit()
{
   ...
   if(g_expert == NULL)
   {
    ...
    if(!g_expert.Init(
       ...
       InpAllowMonday,
       InpAllowFriday,
       InpTrailingStopEnabled,
       InpProfitTrailingPercent,
       InpPanelBgColor,
       InpButtonColor,
       InpPanelX,
       InpPanelY,
